...
select_device() {
    ...
    echo
    read -r -p "Enter keyboard name (e.g., 'MX Keys'): " keyboard_name
    ...
    if [[ -z "$vendor_id" || -z "$product_id" ]]; then
        ...
        read -r -p "VendorID (e.g., 0x046d): " vendor_id
        read -r -p "ProductID (e.g., 0xb35b): " product_id
    else
...
show_menu() {
    ...
    [[ "$choice" != "0" ]] && { read -r -p "Press Enter to continue..."; show_menu; }
}
