...
BACKUP_DIR="${BACKUP_DIR:-$SCRIPT_DIR/configs}"
...
export_aerospace_config() {
    ...
    local aerospace_dir="$AEROSPACE_CONFIG_DIR"
    local backup_dir="${1:-$(init_backup_dir)}"
...
export_kitty_config() {
    ...
    local kitty_conf="$KITTY_CONFIG_DIR/kitty.conf"
    local backup_dir="${1:-$(init_backup_dir)}"
...
export_yazi_config() {
    ...
    local yazi_dir="$YAZI_CONFIG_DIR"
    local backup_dir="${1:-$(init_backup_dir)}"
...
export_all_configs() {
    ...
    echo "Creating backup in: $BACKUP_DIR/$(basename "$backup_dir")"
    echo
    ...
    if backup_vscode_settings "$backup_dir" && backup_vscode_themes "$backup_dir"; then
        results+=("✅ VS Code")
        ((exported++))
    else
        results+=("❌ VS Code")
    ...
    if backup_cursor_settings "$backup_dir" && backup_cursor_themes "$backup_dir"; then
        results+=("✅ Cursor AI")
        ((exported++))
    else
        results+=("❌ Cursor AI")
    ...
    if backup_pycharm_settings "$backup_dir"; then
        results+=("✅ PyCharm")
        ((exported++))
    else
        results+=("❌ PyCharm")
    ...
    log_info "Location: $BACKUP_DIR/$(basename "$backup_dir")"
}
...
show_backup_menu() {
    while true; do
        ...
        echo
        echo "7) IDE settings (VS Code, Cursor, PyCharm)"
        echo
        echo "8) Export all"
        echo
        echo "0) Back"
