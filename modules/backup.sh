...
choose_backup_dir() {
    local backup_dir
    backup_dir="$(get_expanded_config "BACKUP_DIR")/configs"
    ls -la "$backup_dir"
}
...
restore_pycharm_settings() {
	...
	local latest_pycharm_dir
	latest_pycharm_dir=$(find "$pycharm_config_dir" -name "PyCharm*" -not -name "*-backup*" -type d | sort -V | tail -1)

	if [[ -z "$latest_pycharm_dir" ]]; then
		log_error "No PyCharm installation directory found under $pycharm_config_dir — open PyCharm once before restoring"
		return 1
	fi
