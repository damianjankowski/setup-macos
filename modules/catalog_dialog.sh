...
_build_checklist_tsv() {
    ...
    yq -o=json "$catalog_path" | jq -r --arg cat "$category" '
      ...
      | map({
          id: (if (.type == "krew") then "krew:" + (.id // .name // .slug) elif (.type == "pipx") then "pipx:" + (.id // .name // .slug) elif (.type == "curl") then "curl:" + (.id // .name // .slug) elif (.brew_type == "cask") then "cask:" + (.id // .name // .slug) else (.id // .name // .slug) end),
...
select_packages_from_catalog() {
    ...
    args=()
    while IFS=$'\t' read -r tag item def; do
        args+=("$tag" "$item" "$def")
    done <<< "$tsv"
