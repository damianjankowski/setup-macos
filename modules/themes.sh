...
# Catppuccin themes installation module
...
install_yazi_themes() {
    ...
    # Default to mocha or first available
    local selected_theme
    if [[ "$choice" =~ ^[0-9]+$ ]] && [[ "$choice" -ge 1 ]] && [[ "$choice" -le "${#themes[@]}" ]]; then
        selected_theme="${themes[$((choice-1))]}"
    else
        # Find mocha or use first
...
configure_delta_theme() {
    ...
    local delta_dir="$HOME/.config/delta"
